          ...
          <div v-if="themeMenuOpen" class="theme-backdrop" @click="themeMenuOpen = false">
            <div class="theme-menu" @click.stop>
              ...
              <div class="theme-grid">
                <button v-for="t in themes" :key="t.id" class="theme-option" :class="{ active: activeTheme === t.id }"
...
const { clearCredential } = useBiometric()
...
const initials = computed(() => {
  ...
  return userName.value.split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase()
})

function confirmLogout() {
  if (confirm('¿Cerrar sesión?')) {
    logout()
    //clearCredential()   // ← borra también la huella registrada
    router.push('/login')
  }
}
...
.av-ac {
  background: rgba(252, 107, 107, 0.12);
}
...
.tag-red {
  background: rgba(252, 107, 107, 0.15);
  color: #fc6b6b;
}
...
.call-btn {
  ...
  border: 1.5px solid rgba(56, 217, 169, 0.25);
  color: var(--accent2);
  border-radius: var(--radius);
  padding: 13px;
  font-size: 0.9rem;
...
.logout-btn:active { color: var(--danger); background: rgba(252,107,107,0.1); }
</style>